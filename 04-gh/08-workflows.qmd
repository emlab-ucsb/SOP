...
5. **Open a pull request, review it yourself, and merge.** Even for solo work, looking at your own diff before merging is a useful moment to catch mistakes or reconsider your approach. On a solo project, a brief self-review is fine. Merge using **Squash and merge** to keep the `main` history tidy, then delete the branch. When working alone, enabling a Copilot review can also be a useful first pass.

6. **Repeat.** Pull the latest `main`, start the next branch.

The rhythm of this workflow yields a `main` branch whose history reads as a sequence of coherent contributions, with each PR representing one unit of work. This makes it easier to locate when and why a specific change was made, months or years later.
...
5. Address any review comments. Depending on the ruleset, you may need to re-request review to get approval before merging. 
6. Once you have an approved version ready for deployment, merge the PR. 
7. Check that Issues associated with the PR were closed automatically.
