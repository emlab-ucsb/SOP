...
Reproducible research requires not just the same code, but the same computational environment. To ensure that our analyses can be replicated years later or shared across different machines today, emLab adheres to a tiered strategy for managing software versions. This prevents the common problem of code breaking when a package is updated or a system dependency changes.

## R

For R projects, we manage three layers: the R version itself, the project-specific library, and the package source. Managing each separately ensures that a global R installation update doesn't break a legacy project.
...
Every emLab project should have its packages isolated from the rest of your system. We use [**renv**](https://rstudio.github.io/renv/articles/renv.html) to create a private library of packages for each project.
...
* **Daily Workflow:**
  ...
  * `renv::restore()`: When pulling changes from GitHub, run this to sync your local library with the project's lockfile. This rebuilds the environment on a colleague's computer.
...
* **Mixing Pip and Conda:** You use `pip install` inside a Mamba/Conda environment for a package that has complex C-dependencies.
    * **The Consequence:** Pip and Conda do not communicate well; Pip might overwrite a library that Conda relies on, leading to a broken environment that won't update.
