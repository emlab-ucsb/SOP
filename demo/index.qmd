...
::: {.where}
Good to know — but Danielle says branch from issue!!!
:::
...
merges** — so add it here. One PR can close several issues:
`Closes #12, closes #15`. (Keep PRs small and focused; not ready yet? Open a
**Draft PR**.)
:::
...
## Or: the reviewer edits it themselves {.smaller}

::: {.where}
[WATCH]{.badge-watch} (reviewer's option) &nbsp; [github.com]{.tool-gh}
:::

Instead of the request-changes round-trip, for a **tiny** fix the reviewer can
just make the change themselves, then approve:

1. **Files changed** → click the **pencil ✏️** to edit the file in the browser.
2. Tweak it → **Commit changes** to the PR's branch.
3. **Review changes → Approve → Submit**, then author can merge pull request.

::: {.panel}
Talk with your team about expectations for when reviewers should make changes themselves. 
For anything substantial, leave a comment so the **author** makes the change and keeps
ownership.
:::

## Author makes the change {.smaller}
...
greet <- function(name) {
  paste0("The best state in the nation, 
  New Jersey, welcomes you, ", name, "! 
    We can't wait for you to taste a bagel.")
}
```
...
![](images/country-nj.png){width="300" fig-align="center"}

::: {.caption}
Art: @livinggoodstudios on Instagram
:::

## Reviewer approves, author merges {.smaller}
...
::: {.where}
The most important tool isn't on this list — it's **communication**
:::

Git coordinates *code* — not your team. Agree on the **conventions** up front.
...
A conflict happens when **two people change the same line**. You'll create one
on purpose with your partner, in your pair's **shared file** (e.g. `R/danielle-tracey.R`).

It takes three steps:

1. Put a **shared file** on `main`.
2. **Both branch** from the updated `main`.
3. **Both edit the same line** — the second PR will conflict.

**First, decide who is Partner A and who is Partner B.**
...
## Conflict: Step 1 — create the shared file {.smaller}

::: {.where}
[EVERYONE]{.badge-everyone} (in your pairs) &nbsp; [Positron]{.tool-pos} → [github.com]{.tool-gh}
:::

Get one shared file onto `main` so you both have it:

1. **Partner A:** new branch → add the file → commit → push → open a PR.
...
2. **Partner B:** review → **Approve**.
3. **Partner A:** **merge** → delete the branch.

The file is now on `main`.

## Conflict: Step 2 — both branch from `main` {.smaller}

::: {.where}
[EVERYONE]{.badge-everyone} (in your pairs) &nbsp; [Positron]{.tool-pos} / [RStudio]{.tool-rs}
:::

**Both partners:** update your local `main`, then create your **own** branch
from it (A and B on separate branches).

Reminder — update `main` first:

- **Positron / VS Code / RStudio:** switch to `main` in the branch picker → **pull / sync**.
- **Terminal:** `git switch main && git pull`.

A fresh `main` is what makes the next step line up into a clean conflict.

## Conflict: Step 3 — edit the same line {.smaller}

::: {.where}
[EVERYONE]{.badge-everyone} (in your pairs) &nbsp; [Positron]{.tool-pos} → [github.com]{.tool-gh}
:::

Now both of you change the **same line** of the file — differently:

1. **Partner A:** edit the line → commit → push → PR → **Partner B approves. Merge** to `main`.
2. **Partner B:** on *your* branch, edit the **same line** to something else →
   commit → push → **open your PR**.

::: {.panel}
`main` now has A's version and B's branch has a different one on the **same
line** → GitHub **can't auto-merge** B's PR. **Partner B resolves it** (next).
:::
...
![](images/positron-settings-import-merge.png){width="640" fig-align="center"}
