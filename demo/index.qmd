...
A lighter-touch review: a **new branch (not from an issue)**, then the reviewer
...
partner** in your **pair's own file** — e.g. `R/pair-3.R`:

```r
project_version <- "0.1.0"
```

1. Decide who is **Partner A** and **Partner B**. Each makes their own branch
   from `main`.
2. **Partner A:** change the line to `"0.2.0"`, open a PR, and **merge it to
   `main` first**.
3. **Partner B:** on their branch, set the **same line** to a different value
   (e.g. `"0.9.0"`), commit, push, and open a PR.

Now `main` (A's `0.2.0`) and Partner B's branch (`0.9.0`) changed the **same
...
## Pulling `main` in is not just "pull" {.smaller}

::: {.where}
Concept — important for resolving conflicts
:::

::: {.panel}
**Clicking "Pull" / "Sync" only updates your *current* branch from its own
remote.** It does **not** bring `main`'s new commits into your feature branch.
To get `main`'s changes into your branch you must **merge `main` into it**.
:::

You don't have to do that in the terminal:

- **Easiest (today): let the PR show the conflict on github.com** — see next slide.
- **Positron / VS Code:** Command Palette → **Git: Merge Branch…** → pick `main`.
...
## Avoiding painful conflicts {.smaller}

::: {.panel .tight}
- **Bring `main` into your branch often** while you work — small, frequent merges
  beat one giant one at the end.
- **Keep branches short-lived and focused** — one issue, one branch.
- **Keep PRs small** so overlapping edits are rare.
- Coordinate when you know two people are touching the same file.
:::

The longer a branch diverges from `main`, the harder the eventual merge — so
